...
jQCloud.prototype = {
    ...
    drawOneWord: function(index, word) {
        ...
        if (this.options.template) {
            ...
            word_span.append($('<a>').attr(word.link).attr(word.onclick).text(word.text));
        } else if (word.onclick) {
            word.link = "javascript:void(0);";
            ...
            if (typeof word.onclick === 'string') {
                word.onclick = { onclick: word.onclick };
            }
            if (this.options.encodeURI) {
                word.onclick.onclick = encodeURI(word.onclick.onclick).replace(/'/g, '%27');
            }
